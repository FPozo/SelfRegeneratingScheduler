...
class Network:
    ...
    __max_link_real_utilization = None
    ...
    def add_link(self, source, destination, link_type, speed):
        """
    ...
    def remove_link(self, link):
        ...
        :rtype: None
        """
        source = self.__links[link][0]
        destination = self.__links[link][1]
        self.__graph.remove_edge(source, destination)
        # Remove them from the link lists making them None (so we do not change ids during runtime)
        self.__links[link] = None
        # self.__links_objects[link] = None     # I need this information, so for now I will not remove it

    def get_all_simple_paths(self, source, destination):
        """
        ...
        """
        paths = []
        for node_path in nx.all_simple_paths(self.__graph, source, destination):
            first_iteration = False
            previous_node = None
            path = []
            abort = False  # Due to all simple paths doing indirect graphs, if we not find the obtained path
            # we just abort and go for the next path
            for node in node_path:
                if not first_iteration:
                    first_iteration = True
                else:  # Find the index in the link list with the actual and previous node
                    if [previous_node, node] not in self.__links:
                        abort = True
                        break
    ...
    def get_link_source_destination(self, link):
        """
        ...
        :rtype: int, int
        """
        return self.__links[link][0], self.__links[link][1]

    def get_link_by_source_destination(self, source, destination):
        """
        ...
        :rtype: int
        """
        return self.__links.index([source, destination])
    ...
    def get_link_stats(self, link):
        ...
        :rtype: int, LinkType
        """
        return self.__links_objects[link].get_speed(), self.__links_objects[link].get_type()

    def get_min_time_switch(self):
        """
        Get the min_time_switch of the network
        :return: min_time_switch
        :rtype: int
        """
        return self.__min_time_switch

    def set_min_time_switch(self, min_time_switch):
        """
        ...
        :rtype: None
        """
        self.__min_time_switch = min_time_switch
    ...
    def set_time_between_frames(self, time_between_frames):
        ...
        :rtype: None
        """
        self.__time_between_frames = time_between_frames
    ...
    def get_protocol(self):
        ...
        :rtype: int, int
        """
        return self.__period_protocol, self.__time_protocol

    def set_protocol(self, period, time):
        """
        ...
        :rtype: None
        """
        self.__period_protocol = period
        self.__time_protocol = time

    def get_links_connection(self):
        """
    ...
    def __add_link_information(self, links, num_links, branch, parent_node):
        ...
        self.add_link(parent_node, self.__graph.number_of_nodes() - 1, link_type, speed)
    ...
    def __create_cyclic_topology(self):
        ...
        for node_id, node in enumerate(self.__cyclic_description):
            for connection in node:
                ...
                self.add_link(node_id, connection[0], link_type, speed)
    ...
    def generate_paths_frames(self):
        """
        ...
        self.__max_link_real_utilization = 0
        self.__link_utilization = [0 for _ in range(len(self.__links))]
        self.__frame_instances = 0
        real_link_utilization = [0 for _ in range(len(self.__links))]
        ...
        if self.__period_protocol is not None:
            for index, link in enumerate(self.__links_objects):
                ...
                real_link_utilization[index] = int((self.__hyper_period / self.__period_protocol) *
                                                   (self.__time_protocol + self.__time_between_frames))
        ...
        for index_frame, frame in enumerate(self.__frames):
            ...
            for receiver in frame.get_receivers():
                ...
                for id_path, path in enumerate(paths):
                    for link in path:
                        if not self.__frames[index_frame].link_in_path(link):
                            paths_cost[id_path] += int(((frame.get_size() * 1000) /
                ...
                for link in paths[min_index]:
                    if not self.__frames[index_frame].link_in_path(link):
                        self.__link_utilization[link] += int(((frame.get_size() * 1000) /
                                                              self.__links_objects[link].get_speed()) *
                                                             (self.__hyper_period / frame.get_period()))
                        real_link_utilization[link] += int((((frame.get_size() + self.__time_between_frames) * 1000)
                                                            / self.__links_objects[link].get_speed()) *
                                                           (self.__hyper_period / frame.get_period()))
        ...
        for id_link in range(len(self.__link_utilization)):
            ...
            real_link_utilization[id_link] = float(real_link_utilization[id_link])
            self.__link_utilization[id_link] /= self.__hyper_period
            real_link_utilization[id_link] /= self.__hyper_period
        ...
        self.__max_link_real_utilization = max(real_link_utilization)
    ...
    def add_custom_frame(self, sender, receivers, period, deadline, size, starting, end_to_end):
        """
        ...
        :rtype: None
        """
        self.__frames.append(Frame(sender, receivers, period, deadline, size, starting, end_to_end))
        # We need to add the different periods to the period list in order to calculate the hyper period
    ...
    def __calculate_utilization(self):
        ...
        link_utilization = []
        fake_link_utilization = []
        for _ in self.__links:
            link_utilization.append(0)
            fake_link_utilization.append(0)
        ...
        for frame in self.__frames:
            ...
            for link in unique_links:
                ...
                link_utilization[link] += int(((frame.get_size() * 1000000) / self.__links_objects[link].get_speed()) *
                                              (self.__hyper_period / frame.get_period()))
                fake_link_utilization[link] += int((((frame.get_size() * 1000000) + self.__time_between_frames) /
                                                    self.__links_objects[link].get_speed()) *
                                                   (self.__hyper_period / frame.get_period()))
        ...
        if self.__period_protocol is not None:
            for index, link in enumerate(self.__links_objects):
                ...
                fake_link_utilization[index] += int((self.__hyper_period / self.__period_protocol) *
                                                    (self.__time_protocol + self.__time_between_frames))
                self.__frame_instances += int((self.__hyper_period / self.__period_protocol))
        ...
        for index, link in enumerate(link_utilization):
            ...
            fake_link_utilization[index] /= self.__hyper_period
            if link_utilization[index] > 1.0:  # Check if is possible to schedule all of its links
                possible = False
            if fake_link_utilization[index] > 1.0:
                possible_fake = False
            utilization += link_utilization[index]
        return utilization / len(link_utilization), max(link_utilization), possible, possible_fake
    ...
    def write_network_xml(self, name):
        ...
        Xml.SubElement(general_information_xml, 'RealMaximumLinkUtilization').text = \
            str(self.__max_link_real_utilization)
